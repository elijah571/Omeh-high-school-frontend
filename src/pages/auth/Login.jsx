...
export const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response =
        userType === "admin"
          ? await loginAdmin(userData).unwrap()
          : await loginTeacher(userData).unwrap();

      // Save user credentials to Redux store and localStorage
      dispatch(setCredentials(response));

      console.log(`${userType.charAt(0).toUpperCase() + userType.slice(1)} Logged in:`, response);
  ...
  return (
    <div className="login-container">
      ...
      <select className="user-type-select" onChange={(e) => setUserType(e.target.value)} value={userType}>
      ...
      <form className="login-form" onSubmit={handleLogin}>
        ...
        <button className="login-button" type="submit" disabled={adminLoading || teacherLoading}>
